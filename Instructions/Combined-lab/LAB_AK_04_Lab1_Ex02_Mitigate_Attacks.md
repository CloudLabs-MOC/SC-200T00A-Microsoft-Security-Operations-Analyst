...
1. In the **Device Management** section, select **Onboarding**,  make sure "Windows 10 and 11" is selected as operating system and verify that the **"First device onboarded"** message now shows **Completed**.
...
1. In the Microsoft Defender XDR portal expand **Investigation & responce (1)** from the left menu bar, then expand **Incidents & alerts (2)** and select **Alerts (3)**.

    ![Picture 1](../Media/lab4-06-13.png)
...
1. In the **Alerts** pane, select the alert named **[TestAlert]Suspicious PowerShell commandline** to load its details.
   ...
   ![Picture 1](../Media/lab4-06-14.png)
...
1. In the Microsoft Defender XDR portal select **Incidents & alerts** from the left menu bar, then select **Incidents (1)**

1. Clear the **Alert severity** filter by selecting the **X** on the right of the filter.

1. A new incident called **Hands-on keyboard attack** or **Multi-stage incident... (2)** appears in the right pane. Select the incident name to load its details.

    > **Note:** It may take **10–15 minutes** for the detection test alert to be processed and an incident to be generated. If the incident doesn't appear immediately, refresh the **Incidents** page (**Ctrl+F5**) and clear any active filters. Depending on the Microsoft Defender XDR version and automatic incident correlation, the alert may be associated with an incident that has a different name.

1. Select the **Manage incident (3)** link (with a pencil icon) and a new window blade appears.

    ![Picture 1](../Media/page17-t1p3.png)
...
1. In the Microsoft Defender XDR portal expand **Investigation & responce** from the left menu bar, then expand **Incidents & alerts** and select **Incidents (1)**.

   >**Note:** On updated versions of the Microsoft Defender XDR portal page **Incidents & alerts** are found under the **Investigations & response** menu heading.

1. A new incident called **Multi-stage incident involving Defense evasion & Discovery on one endpoint (2)** is in the right pane. Select the incident name to load its details.

   >**Note:** If you do not see the incident, make sure to clear all the filter by selecting the **X** on the right of the filter.

    ![Picture 1](../Media/lab4-06-15.png)

    > **Note:** It may take **10–15 minutes** for the incident to appear. If it isn't visible, refresh the **Incidents** page (**Ctrl+F5**) and clear any active filters. 
...
1. Re-expand the **Alerts** pane (left-side) and select the **Play attack story** **Run** icon. This shows the attack timeline alert by alert and dynamically populates the *Incident graph*.
   ...
   ![Picture 1](../Media/lab4-06-16.png)
