...
## Architecture Diagram
  ...
  ![Picture 1](../Media/sc200-l6-arch.png)
...
1. In the query editor, enter the following query **(1)** and select the **Run query (2)** button. You should see the query results in the bottom window.

    ```KQL
    SecurityEvent_CL
    ```

    ![](../Media/sc200-l6-02.png)

    > **Note:** Notice that the default results render a Timeline visualization chart.

    >**Note:** It may take 5 minutes for the query results to appear.
...
1. The following statement demonstrates the **mv-apply** operator, which applies a subquery to each record and returns the union of the results of all subqueries.
    ...
    >**Note:** A **function** is a log query that can be used in other log queries with the saved name as a command. 

1. To create a **function**, after running your query, select the **Save (1)** button and then select **Save As function (2)** from the drop-down. 
...
1. Select **Save (3)**. 
    ...
    ![](../Media/sc200-l6-08.png)

1. Expand the *side panel* if collapsed, and use the drop down to select **Functions (1)** and then select the folder under **My functions (2)** *where you saved your function. You should see the function you just created.
